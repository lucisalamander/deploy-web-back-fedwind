...
# ============================================================================
declare -a STRATEGY=(scaffold fedln fedprox fedper)
        ...
        for batch_size in "${BATCH_SIZE[@]}"; do
          for num_clients in "${NUM_CLIENTS[@]}"; do
            for strategy in "${STRATEGY[@]}"; do
              # Set proximal_mu based on strategy
              if [[ "$strategy" == "fedprox" ]]; then
                proximal_mu_values=("${PROXIMAL_MU[@]}")
              else
                proximal_mu_values=(0)
              fi

              for proximal_mu in "${proximal_mu_values[@]}"; do
